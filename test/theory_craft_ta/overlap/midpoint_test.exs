defmodule TheoryCraftTA.Overlap.MIDPOINTTest do
  ...
  describe "midpoint/2 with list input" do
    ...
    test "handles NaN in middle (invalid data scenario)" do
      ...
      # Python result: [nan nan 2. 2.5 4. nan 6. 7.]
      # MIDPOINT is special - it doesn't fully propagate NaN like other indicators
      assert {:ok, result} = MIDPOINT.midpoint(data, 3)
      assert Enum.at(result, 0) == nil
      assert Enum.at(result, 1) == nil
      assert Enum.at(result, 2) == 2.0
      assert Enum.at(result, 3) == 2.5
      assert Enum.at(result, 4) == 4.0
      assert Enum.at(result, 5) == nil
      assert Enum.at(result, 6) == 6.0
      assert Enum.at(result, 7) == 7.0
    end
  end
  ...
  describe "init/1" do
    test "initializes with valid parameters" do
      assert {:ok, _state} =
               MIDPOINT.init(period: 14, data: "eurusd_m1", name: "midpoint14", source: :close)
    end

    test "returns error for period < 2" do
      assert {:error, msg} =
               MIDPOINT.init(period: 1, data: "eurusd_m1", name: "midpoint1", source: :close)
  ...
  describe "next/2 with Bar input" do
    test "processes bars correctly in APPEND mode" do
      {:ok, state} =
        MIDPOINT.init(period: 2, data: "eurusd_m1", name: "midpoint2", source: :close)
  ...
  describe "property: state-based APPEND matches batch calculation" do
    property "APPEND mode matches batch MIDPOINT" do
      check all(
        ...
        {:ok, initial_state} =
          MIDPOINT.init(period: period, data: "test", name: "midpoint", source: :close)
