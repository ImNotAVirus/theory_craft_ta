defmodule TheoryCraftTA.Overlap.SMATest do
  ...
  describe "sma/2 with list input" do
    ...
    test "handles period=2 (minimum valid)" do
      ...
      assert {:ok, result} = SMA.sma(data, 2)
  ...
  describe "sma/2 with DataSeries input" do
    test "maintains DataSeries type in output" do
      ...
      # SMA result should be [4.0, 3.0, 2.0, nil, nil] (newest-first)
    ...
    property "UPDATE recalculates with replaced last value" do
      check all(
        ...
        Enum.reduce(update_values, {final_state, data}, fn update_value, {state, current_data} ->
          ...
          {:ok, batch_result} = SMA.sma(updated_data, period)
          batch_sma = List.last(batch_result)

          # State UPDATE should match batch calculation
          case {state_sma, batch_sma} do
            {nil, nil} ->
              :ok
