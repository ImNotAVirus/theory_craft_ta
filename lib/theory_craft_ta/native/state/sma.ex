defmodule TheoryCraftTA.Native.State.SMA do
  @moduledoc """
  ...
  ## Usage
      ...
      {:ok, state} = TheoryCraftTA.Native.State.SMA.init(14)

      # Process first bar
      {:ok, sma1, state2} = TheoryCraftTA.Native.State.SMA.next(state, 100.0, true)

      # Process second bar
      {:ok, sma2, state3} = TheoryCraftTA.Native.State.SMA.next(state2, 110.0, true)

      # Update same bar (multiple ticks)
      {:ok, sma3, state4} = TheoryCraftTA.Native.State.SMA.next(state3, 105.0, false)
  ...
  ## Examples
      ...
      iex> {:ok, _state} = TheoryCraftTA.Native.State.SMA.init(14)
  ...
  ## Examples
      ...
      iex> {:ok, state} = TheoryCraftTA.Native.State.SMA.init(2)
      iex> {:ok, nil, state2} = TheoryCraftTA.Native.State.SMA.next(state, 100.0, true)
      iex> {:ok, sma, _state3} = TheoryCraftTA.Native.State.SMA.next(state2, 110.0, true)
      iex> sma
      105.0
