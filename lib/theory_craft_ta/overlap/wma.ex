defmodule TheoryCraftTA.Overlap.WMA do
  ...
  with `TheoryCraft.MarketSimulator`:
      ...
      simulator =
        ...
        |> MarketSimulator.add_indicator(TA.wma(eurusd_m5[:close], 20, name: "wma20"))
  ...
  - `opts` - Keyword list with:
    - `:period` (required) - The WMA period (must be >= 2)
  ...
  ## Examples
      ...
      iex> {:ok, _state} = TheoryCraftTA.Overlap.WMA.init(period: 14, data: "eurusd", name: "wma14", source: :close)
  ...
  @doc """
  Processes a MarketEvent and calculates the next WMA value.
  ...
  - `{:ok, indicator_value, new_state}` - IndicatorValue with WMA calculation
  ...
  def next(%MarketEvent{} = event, %WMA{} = state) do
    %WMA{
      source: source,
      data_name: data_name,
      state: native_state
    ...
    {:ok, {wma_value, new_native_state}} =
      Native.overlap_wma_state_next(native_state, value, is_new_bar)

    new_state = %WMA{state | state: new_native_state}
