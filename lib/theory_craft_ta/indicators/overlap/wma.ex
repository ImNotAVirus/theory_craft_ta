defmodule TheoryCraftTA.Indicators.Overlap.WMA do
  @moduledoc """
  Weighted Moving Average (WMA) indicator implementing TheoryCraft.Indicator behaviour.
  ...
  ## Usage with TheoryCraft
      ...
      |> MarketSimulator.add_indicator(
        TheoryCraftTA.Indicators.Overlap.WMA,
        period: 20,
        data: "eurusd_m5",
        name: "wma20",
        source: :close
  ...
  alias TheoryCraftTA.Overlap.WMA, as: WMACore
  ...
  def init(opts) do
    ...
    {:ok, indicator_state} = WMACore.init(period)
  ...
  def next(%MarketEvent{} = event, %WMA{} = state) do
    ...
    case WMACore.next(value, is_new_bar, indicator_state) do
      {:ok, wma_value, new_indicator_state} ->
        new_state = %WMA{state | indicator_state: new_indicator_state}

        updated_data = Map.put(event.data, output_name, wma_value)
